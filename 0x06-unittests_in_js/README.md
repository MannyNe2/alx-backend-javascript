...
+ [x] 0. **Basic test with Mocha and Node assertion library**
  ...
  + **Run test:**
    ```powershell
    manny@Amanns:~$ npm test 0-calcul.test.js
...
+ [x] 8. **Basic Integration testing**
  ...
  + **Server:**
    + Terminal 1:
      ```powershell
      manny@Amanns:~/8-api$  node api.js
      API available on localhost port 7865
      ```
    + Terminal 2:
      ```powershell
      manny@Amanns:~/8-api$  curl http://localhost:7865 ; echo ""
      Welcome to the payment system
      manny@Amanns:~/8-api$
      manny@Amanns:~/8-api$ npm test api.test.js
...
+ [x] 9. **Regex integration testing**
  ...
  + **Server:**
    + Terminal 1:
      ```powershell
      manny@Amanns:~$ node api.js
      API available on localhost port 7865
      ```
    + Terminal 2:
      ```powershell
      manny@Amanns:~$ curl http://localhost:7865/cart/12 ; echo ""
      Payment methods for cart 12
      manny@Amanns:~$
      manny@Amanns:~$ curl http://localhost:7865/cart/hello -v
      *   Trying 127.0.0.1...
      * TCP_NODELAY set
...
+ [x] 10. **Deep equality & Post integration testing**
  ...
  + **Server:**
    ...
    + Terminal 2:
      ```powershell
      manny@Amanns:~$ curl http://localhost:7865/available_payments ; echo ""
      {"payment_methods":{"credit_cards":true,"paypal":false}}
      manny@Amanns:~$
      manny@Amanns:~$ curl -XPOST http://localhost:7865/login -d '{ "userName": "Betty" }' -H 'Content-Type: application/json' ; echo ""
      Welcome Betty
      manny@Amanns:~$
      ```
  + **Tips:**
